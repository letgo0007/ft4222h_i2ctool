 ...
 *  2. gcc main.c i2cm.c -Wall -Wextra -L. -lft4222 -Wl,-rpath,/usr/local/lib -o ft4222h_i2ctool
 *  3. sudo ./a.out
 */
#include <stdio.h>
#include <stdlib.h>
...
static char slogan1[EEPROM_BYTES + 1] = "FTDI Chip strives to Make Design Easy with our modules, cables "
        "and integrated circuits for USB connectivity and display systems.";

static char slogan2[EEPROM_BYTES + 1] = "FT4222H: Hi-Speed USB 2.0 QSPI/I2C device controller.  QFN32, "
...
static FT4222_STATUS setWordAddress(FT_HANDLE ftHandle, const uint16 slaveAddr, uint8 wordAddress)
{
    FT4222_STATUS ft4222Status;
    uint16 bytesToWrite = 1;
    uint16 bytesWritten = 0;

    ft4222Status = FT4222_I2CMaster_Write(ftHandle, slaveAddr, &wordAddress, bytesToWrite, &bytesWritten);
 ...
 */
static FT4222_STATUS pollAddressAck(FT_HANDLE ftHandle, const uint16 slaveAddr)
{
    ...
    for (tries = 0; tries < 25; tries++)
    {
        ...
        ft4222Status = setWordAddress(ftHandle, slaveAddr, 0);
        if (FT4222_OK != ft4222Status)
        {
            return ft4222Status;
        }
...
static int exercise4222(DWORD locationId)
{
    ...
    ft4222Status = FT4222_I2CMaster_Read(ftHandle, slaveAddr, originalContent, bytesToRead, &bytesRead);
    if (FT4222_OK != ft4222Status)
    {
    ...
    for (page = 0; page < EEPROM_BYTES / BYTES_PER_PAGE; page++)
    {
        ...
        memcpy(&pageBuffer[1], writeBuffer + page * BYTES_PER_PAGE, BYTES_PER_PAGE);

        bytesToWrite = BYTES_PER_PAGE + 1;
        ft4222Status = FT4222_I2CMaster_Write(ftHandle, slaveAddr, pageBuffer, bytesToWrite, &bytesWritten);
        if (FT4222_OK != ft4222Status)
        {
    ...
    ft4222Status = FT4222_I2CMaster_Read(ftHandle, slaveAddr, newContent, bytesToRead, &bytesRead);
    if (FT4222_OK != ft4222Status)
    {
        printf("FT4222_I2CMaster_Read failed (error %d)\n", (int) ft4222Status);
        goto exit;
    ...
    exit: if (ftHandle != (FT_HANDLE) NULL)
    {
    ...
    for (i = 0; i < (int) numDevs; i++)
    {
        ...
        if (devType == FT_DEVICE_4222H_0)
        {
            ...
            if ('A' == devInfo[i].Description[descLen - 1])
            {
                ...
                printf("  0x%08x  %s  %s\n", (unsigned int) devInfo[i].ID, devInfo[i].SerialNumber,
